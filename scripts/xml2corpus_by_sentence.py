...
def flatten_body(body: list, id: int, name: str) -> pd.DataFrame:
    ...
    Args:
        ...
        id (int): Identifier of the paper.
    ...
    # df_body['sentence_text'] = df_body['paragraph'].str.split(r"(?<=[.!?])\s+(?=[A-Z])")

    ### Start of regex that was generated by co-pilot ###
    # Regex to split sentences, but avoid splitting after common
    # personal/academic titles (e.g., "Dr.", "Mr.", etc.)
    # We'll use a non-capturing group for the titles and match only
    ...
    # and a capital letter, then post-process to merge splits after titles.
    regex = r"(?<=[.!?])\s+(?=[A-Z])"
    df_body['sentence_text'] = df_body['paragraph'].str.split(regex)
    # Post-process to merge sentences that were split after known titles
    def merge_abbrev_splits(sentences):
        ...
        for i, sent in enumerate(sentences):
            ...
            if i > 0:
                ...
                # Check if previous sentence ends with a known title abbreviation
                if re.search(rf"\b{titles}\.$", prev):
...
if __name__ == "__main__":
    ...
    for id, name in enumerate(selected_xml_papers):
        if Path(f"data/xml_papers/{name}.xml").exists():
            ...
            df_body = flatten_body(body, id, name)
